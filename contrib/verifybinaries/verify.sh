...
###   sphinxcoincore.org and sphinxcoin.org and compares them.
...
WORKINGDIR="/tmp/sphinxcoin_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://sphinxcoincore.org"
HOST2="https://sphinxcoin.org"
BASEDIR="/bin/"
VERSIONPREFIX="sphinxcoin-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'sphinxcoin-',
...
if [ $? -ne 0 ]; then
   echo "sphinxcoin.org failed to provide signature file, but sphinxcoincore.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "sphinxcoin.org and sphinxcoincore.org signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Sphinxcoin Core binary release signing key installed?"
   fi
