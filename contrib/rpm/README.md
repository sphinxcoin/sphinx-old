...
The RPM spec file provided here is for Sphinxcoin-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://sphinxcoin.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[Sphinxcoin git repository](https://github.com/sphinxcoin/sphinxcoin) but are not part of
...
Currently only `Source10` is used, for the example `sphinxcoin.conf` file.
...
in the sphinxcoin git repository. At present only an SVG version of the Sphinxcoin
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the sphinxcoin
...
needed when 0.12.1 is released, a proper fix is already in the Sphinxcoin git
master branch.
...
idea to build Sphinxcoin Core against a static version of that library compiled
...
The `sphinxcoin.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `sphinxcoin-server` package. At this point, most RPM based
...
The `sphinxcoin-server` package should have SELinux support. How to properly do
...
Sphinxcoin-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/sphinxcoin/). He has been
packaging Sphinxcoin for Fedora at least since 2012.
...
4. I renamed the `sphinxcoin` package that contains the Qt GUI to `sphinxcoin-core` as
that appears to be how the general population refers to it, in contrast to
`sphinxcoin-xt` or `sphinxcoin-classic`. I wanted to make sure the general population
