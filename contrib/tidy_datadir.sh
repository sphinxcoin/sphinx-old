#!/bin/bash
# Copyright (c) 2013 The Sphinxcoin Core developers
...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete Sphinxcoin database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no Sphinxcoin datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old Sphinxcoin datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected Sphinxcoin 0.7 datadir."
    ;;
  3)
    echo "Detected Sphinxcoin pre-0.8 datadir."
    ;;
  4)
    echo "Detected Sphinxcoin 0.8 datadir."
    ;;
esac
