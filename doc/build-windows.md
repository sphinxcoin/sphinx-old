...
Below are some notes on how to build Sphinxcoin Core for Windows.

The options known to work for building Sphinxcoin Core on Windows are:

* On Linux using the [Mingw-w64](https://mingw-w64.org/doku.php) cross compiler tool chain. Ubuntu Trusty 14.04 is recommended
and is the platform used to build the Sphinxcoin Core Windows release binaries.
...
for Ubuntu Xenial does not produce working executables for some of the Sphinxcoin Core applications.
...
Note that for WSL the Sphinxcoin Core source path MUST be somewhere in the default mount file system, for
...
The next three steps are an example of how to acquire the source in an appropriate way.
    ...
    cd /usr/src
    sudo git clone https://github.com/sphinxcoin/sphinxcoin.git
    sudo chmod -R a+rw sphinxcoin
...
way. This will install to `c:\workspace\sphinxcoin`, for example:
...
causes two of the sphinxcoin executables to crash shortly after start up. The bug is related to the
...
It's not possible to build the sphinxcoin code using the win32 version of the Mingw-w64 cross compilers (at least not without
modifying headers in the sphinxcoin source code).
