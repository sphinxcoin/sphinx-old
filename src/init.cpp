...
void Shutdown()
{
    ...
    RenameThread("sphinxcoin-shutoff");
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: Sphinxcoin.conf)") + "\n";
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: Sphinxcoin.pid)") + "\n";
    ...
    strUsage += "\n" + _("SSL options: (see the Sphinxcoin Wiki for SSL setup instructions)") + "\n";
...
strUsage += "\n" + _("Masternode options:") + "\n";
    ...
    strUsage += "  -anonymizeionamount=<n> " + _("Keep N Sphinxcoin anonymized (default: 0)") + "\n";
...
std::string LicenseInfo()
{
    ...
    const std::string URL_WEBSITE = "<https://www.sphinxcoin.org>";
...
/** Sanity checks
 *  Ensure that Sphinxcoin is running in a usable environment with all
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. Sphinxcoin is shutting down."));
...
#endif
    // Make sure only a single Sphinxcoin process is using the data directory.
    boost::filesystem::path pathLockFile = GetDataDir() / ".lock";
    FILE* file = fopen(pathLockFile.string().c_str(), "a"); // empty lock file; created if it doesn't exist.
    if (file) fclose(file);
    static boost::interprocess::file_lock lock(pathLockFile.string().c_str());
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Sphinxcoin is probably already running."), strDataDir));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("Sphinxcoin version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    if (fDaemon) fprintf(stdout, "Sphinxcoin server starting\n"); 
...
#ifdef ENABLE_WALLET
    if (mapArgs.count("-reservebalance")) // sphinxcoin: reserve balance amount
    {
    ...
    // requested to kill sphinxcoin-qt during the last operation. If so, exit.
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of Sphinxcoin") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart Sphinxcoin to complete") << "\n";
    ...
    nAnonymizeSphinxcoinAmount = GetArg("-anonymizeionamount", 0);
    if(nAnonymizeSphinxcoinAmount > 999999) nAnonymizeSphinxcoinAmount = 999999;
    if(nAnonymizeSphinxcoinAmount < 2) nAnonymizeSphinxcoinAmount = 2;
    ...
    LogPrintf("Anonymize Sphinxcoin Amount %d\n", nAnonymizeSphinxcoinAmount);

    /* Denominations
       A note about convertability. Within Stashedsend pools, each denomination
       is convertable to another.
       For example:
       1SPHX+1000 == (.1SPHX+100)*10
       10SPHX+10000 == (1SPHX+1000)*10
    */
