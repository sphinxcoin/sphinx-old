...
void CNode::copyStats(CNodeStats &stats)
{
    ...
    // due to a new block arriving or other large sphinxcoin.
    ...
    // Raw ping time is in microseconds, but show it to user as whole seconds (Sphinxcoin users should be well used to small numbers with many decimal places by now :)
...
void ThreadMapPort()
{
    ...
    if (r == 1)
    {
        ...
        string strDesc = "Sphinxcoin " + FormatFullVersion();
...
bool BindListenPort(const CService &addrBind, string& strError)
{
...
#endif
#ifdef WIN32
        int nProtLevel = 10 /* PROTECTSPHX_LEVEL_UNRESTRICTED */;
        int nParameterId = 23 /* IPV6_PROTECTSPHX_LEVEl */;
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. Sphinxcoin is probably already running."), addrBind.ToString());
        else
