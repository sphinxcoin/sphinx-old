...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc<2 || mapArgs.count("-?") || mapArgs.count("-h") || mapArgs.count("-help")) 
        ...
        std::string strUsage = strprintf(_("%s sphinxcoin-tx utility version"), _(PACKAGE_NAME)) + " " + FormatFullVersion() + "\n\n" +
            _("Usage:") + "\n" +
              "  sphinxcoin-tx [options] <hex-tx> [commands]  " + _("Update hex-encoded sphinxcoin transaction") + "\n" +
              "  sphinxcoin-tx [options] -create [commands]   " + _("Create hex-encoded sphinxcoin transaction") + "\n" +
              "\n";
...
static void MutateTxAddOutAddr(CMutableTransaction& tx, const std::string& strInput)
{
    ...
    CSphinxcoinAddress addr(strAddr);
    if (!addr.IsValid())
...
*/
    if (bScriptHash) {
        ...
        CSphinxcoinAddress redeemScriptAddr(scriptPubKey);
...
static int CommandLineRawTx(int argc, char* argv[])
{
    ...
    try {
        ...
        if (!fCreateBlank) {
            ...
            // param: hex-encoded sphinxcoin transaction
