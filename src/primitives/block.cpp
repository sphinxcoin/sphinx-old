...
bool CBlock::DisconnectBlock(CTxDB& txdb, CBlockIndex* pindex)
{
    ...
    // sphinxcoin: clean up wallet after disconnecting coinstake
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    if (IsProofOfStake())
    {
        // sphinxcoin: coin stake tx earns reward instead of paying fee
        uint64_t nCoinAge;
    ...
    // sphinxcoin: track money supply and mint amount info
...
bool CBlock::AddToBlockIndex(unsigned int nFile, unsigned int nBlockPos, const uint256& hashProof)
{
    ...
    // sphinxcoin: compute chain trust score
    pindexNew->nChainTrust = (pindexNew->pprev ? pindexNew->pprev->nChainTrust : 0) + pindexNew->GetBlockTrust();

    // sphinxcoin: compute stake entropy bit for stake modifier
    ...
    // sphinxcoin: compute stake modifier
...
// ----------- instantX transaction scanning -----------
    ...
    BOOST_FOREACH(const CTransaction& tx, vtx)
    {
        ...
        // sphinxcoin: check transaction timestamp
...
#ifdef ENABLE_WALLET
// sphinxcoin: attempt to generate suitable proof-of-stake
