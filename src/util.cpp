...
int nAnonymizeSphinxcoinAmount = 1000;
...
#else
    const char* pszModule = "sphinxcoin";
#endif
    if (pex)
        return strprintf(
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "sphinxcoin.pid"));
    ...
    if (vTimeOffsets.size() >= 5 && vTimeOffsets.size() % 2 == 1)
    {
        ...
        else
        {
            ...
            if (!fDone)
            {
                ...
                {
                    fDone = true;
                    string strMessage = _("Warning: Please check that your computer's date and time are correct! If your clock is wrong Sphinxcoin will not work properly.");
...
{
    std::string strCopyrightHolders = strPrefix + "The Sphinxcoin Core developers";

    // Check for untranslated substitution to make sure Sphinxcoin Core copyright is not removed by accident
//    if (strprintf(COPYRIGHT_HOLDERS, COPYRIGHT_HOLDERS_SUBSTITUTION).find("Sphinxcoin Core") == std::string::npos) {
//        strCopyrightHolders += "\n" + strPrefix + "The Sphinxcoin Core developers";
//    }
