...
Value stashedsend(const Array& params, bool fHelp)
{
    if (fHelp || params.size() == 0)
        throw runtime_error(
            "stashedsend <sphinxcoinaddress> <amount>\n"
            "sphinxcoinaddress, reset, or auto (AutoDenominate)"
    ...
    if (params.size() != 2)
        throw runtime_error(
            "stashedsend <sphinxcoinaddress> <amount>\n"
            "sphinxcoinaddress, denominate, or auto (AutoDenominate)"
...
Value masternode(const Array& params, bool fHelp)
{
    ...
    if (fHelp  ||
        ...
        throw runtime_error(
                ...
                "  stop         - Stop masternode configured in sphinxcoin.conf\n"
                ...
                "  vote-many    - Vote on a Sphinxcoin initiative\n"
                "  vote         - Vote on a Sphinxcoin initiative\n"
                );
    ...
    if (strCommand == "genkey")
    {
        ...
        return CSphinxcoinSecret(secret).ToString();
    }
...
Value masternodelist(const Array& params, bool fHelp)
{
                ...
                && strMode != "pubkey" && strMode != "rank" && strMode != "status" && strMode != "addr" && strMode != "votes" && strMode != "lastpaid"))
    {
        throw runtime_error(
                ...
                "  votes          - Print all masternode votes for a Sphinxcoin initiative (can be additionally filtered, partial match)\n"
