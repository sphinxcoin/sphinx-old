...
#include "sphinxcoinaddressvalidator.h"
...
{
    widget->setMaxLength(SphinxcoinAddressValidator::MaxAddressLength);
    widget->setValidator(new SphinxcoinAddressValidator(parent));
    widget->setFont(sphinxcoinAddressFont());
}
...
bool parseSphinxcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    // Sphinxcoin: check prefix
    if(uri.scheme() != QString("sphinxcoin"))
        return false;
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if(!SphinxcoinUnits::parse(SphinxcoinUnits::SPHX, i->second, &rv.amount))
                {
                    return false;
                }
            }
...
bool parseSphinxcoinURI(QString uri, SendCoinsRecipient *out)
{
    // Convert sphinxcoin:// to sphinxcoin:
    //
    //    Cannot handle this later, because sphinxcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("sphinxcoin://", Qt::CaseInsensitive))
    {
        uri.replace(0, 11, "sphinxcoin:");
    }
    QUrl uriInstance(uri);
    return parseSphinxcoinURI(uriInstance, out);
}
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "Sphinxcoin.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for Sphinxcoin.lnk
...
{
    return GetAutostartDir() / "sphinxcoin.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        // Write a sphinxcoin.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=Sphinxcoin\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the sphinxcoin app
...
bool GetStartOnSystemStartup()
{
    CFURLRef sphinxcoinAppUrl = CFBundleCopyBundleURL(CFBundleGetMainBundle());
    LSSharedFileListRef loginItems = LSSharedFileListCreate(NULL, kLSSharedFileListSessionLoginItems, NULL);
    LSSharedFileListItemRef foundItem = findStartupItemInList(loginItems, sphinxcoinAppUrl);
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if(fAutoStart && !foundItem) {
        // add sphinxcoin app to startup item list
        LSSharedFileListInsertItemURL(loginItems, kLSSharedFileListItemBeforeFirst, NULL, NULL, sphinxcoinAppUrl, NULL, NULL);
    }
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("sphinxcoin-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  sphinxcoin-Qt [" + tr("command-line options") + "]                     " + "\n";

    coreOptions = QString::fromStdString(HelpMessage(HMM_SPHX_QT));
    ...
    setWindowTitle(tr("sphinxcoin-Qt"));
