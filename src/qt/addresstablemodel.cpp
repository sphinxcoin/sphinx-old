...
QVariant AddressTableModel::data(const QModelIndex &index, int role) const
{
    ...
    else if (role == Qt::FontRole)
    {
        ...
        {
            font = GUIUtil::sphinxcoinAddressFont();
        }
        return font;
    }
...
bool AddressTableModel::setData(const QModelIndex &index, const QVariant &value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case Label:
            ...
            {
                wallet->SetAddressBookName(CSphinxcoinAddress(strTemp).Get(), value.toString().toStdString());
            }
            break;
        case Address:
            ...
            if(CSphinxcoinAddress(rec->address.toStdString()) == CSphinxcoinAddress(value.toString().toStdString()))
            {
                editStatus = NO_CHANGES;
                return false;
            }
            ...
            else if(wallet->mapAddressBook.count(CSphinxcoinAddress(value.toString().toStdString()).Get()))
            {
            ...
            {
                {
                    LOCK(wallet->cs_wallet);
                    // Remove old entry
                    wallet->DelAddressBookName(CSphinxcoinAddress(rec->address.toStdString()).Get());
                    // Add new entry with new address
                    wallet->SetAddressBookName(CSphinxcoinAddress(value.toString().toStdString()).Get(), rec->label.toStdString());
                }
            }
            break;
        }
...
{
    // Update address book model from Sphinxcoin core
...
QString AddressTableModel::addRow(const QString &type, const QString &label, const QString &address, int addressType)
{
    ...
    if(type == Send)
    {
        ...
        } else
        {
            ...
            {
                LOCK(wallet->cs_wallet);
                if (wallet->mapAddressBook.count(CSphinxcoinAddress(strAddress).Get()))
                {
                    editStatus = DUPLICATE_ADDRESS;
                    return QString();
                };
                
                wallet->SetAddressBookName(CSphinxcoinAddress(strAddress).Get(), strLabel);
            }
        }
    }
    else if(type == Receive)
    {
        ...
        if (addressType == AT_Stealth)
        {
            ...
            {
                editStatus = KEY_GENERATSPHX_FAILURE;
                return QString();
            }
        ...
        } else
        {
            ...
            strAddress = CSphinxcoinAddress(newKey.GetID()).ToString();
...
{
    {
        ...
        {
            CSphinxcoinAddress address_parsed(sAddr);
