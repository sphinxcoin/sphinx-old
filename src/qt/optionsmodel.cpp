...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", SphinxcoinUnits::SPHX);
    ...
    if (!settings.contains("nAnonymizeSphinxcoinAmount"))
        settings.setValue("nAnonymizeSphinxcoinAmount", 1000);
    nAnonymizeSphinxcoinAmount = settings.value("nAnonymizeSphinxcoinAmount").toLongLong();
    if (settings.contains("nStashedsendRounds"))
        SoftSetArg("-stashedsendrounds", settings.value("nStashedsendRounds").toString().toStdString());
    if (settings.contains("nAnonymizeSphinxcoinAmount"))
        SoftSetArg("-anonymizeionamount", settings.value("nAnonymizeSphinxcoinAmount").toString().toStdString());
...
QVariant OptionsModel::data(const QModelIndex & index, int role) const
{
    if(role == Qt::EditRole)
    {
        ...
        case AnonymizeSphinxcoinAmount:
            return QVariant(nAnonymizeSphinxcoinAmount);
        case UseBlackTheme:
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case StashedsendRounds:
            ...
            break;
        case AnonymizeSphinxcoinAmount:
            nAnonymizeSphinxcoinAmount = value.toInt();
            settings.setValue("nAnonymizeSphinxcoinAmount", nAnonymizeSphinxcoinAmount);
            emit AnonymizeSphinxcoinAmountChanged(nAnonymizeSphinxcoinAmount);
            break;
        default:
            break;
        }
