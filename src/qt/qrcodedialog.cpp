...
{
    QString ret = QString("sphinxcoin:%1").arg(address);
    int paramCount = 0;
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non SPHX unit input in lnReqAmount, we generate the URI with SPHX as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(SphinxcoinUnits::format(SphinxcoinUnits::SPHX, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
