...
void StashedsendConfig::clickBasic()
{
    ...
    QString strAmount(SphinxcoinUnits::formatWithUnit(
        model->getOptionsModel()->getDisplayUnit(), 1000 * COIN));
    QMessageBox::information(this, tr("Darksend Configuration"),
        tr(
            "Darksend was successfully set to basic (%1 and 2 rounds). You can change this at any time by opening Sphinxcoin's configuration screen."
        ).arg(strAmount)
    );
...
void StashedsendConfig::clickHigh()
{
    ...
    QMessageBox::information(this, tr("Darksend Configuration"),
        tr(
            "Darksend was successfully set to high (%1 and 8 rounds). You can change this at any time by opening Sphinxcoin's configuration screen."
        ).arg(strAmount)
    );
...
void StashedsendConfig::clickMax()
{
    ...
    QMessageBox::information(this, tr("Darksend Configuration"),
        tr(
            "Darksend was successfully set to maximum (%1 and 16 rounds). You can change this at any time by opening Sphinxcoin's configuration screen."
        ).arg(strAmount)
    );
...
void StashedsendConfig::configure(bool enabled, int coins, int rounds) {
    ...
    settings.setValue("nAnonymizeSphinxcoinAmount", coins);

    nStashedsendRounds = rounds;
    nAnonymizeSphinxcoinAmount = coins;
}
